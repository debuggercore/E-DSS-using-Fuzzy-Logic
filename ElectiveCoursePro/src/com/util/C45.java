...
import wekaframework.classfiers.custom.C45Base;

public class C45 extends C45Base {

	DataInstances fullData;
	DataInstances data;
	int MIN_OBJECTS = 5;
	...
	public void setData(DataInstances d) {
		fullData = d;
	}
	 ...
	 */
	RuleGenerationHelper getRules(DataInstances d, String[] newInstance) {
		data = d;
		boolean isInitial = true;
		// rules = "";
		RuleGenerationHelper ruleSet = new RuleGenerationHelper();
		DataInstances currInst = new DataInstances();
		...
		while (loopBreak() == false && n < 5) {
			...
			if (data.getAttributeType(i).trim().compareTo("DISCRETE") == 0) {
				ruleSet.addCondition(new Condition(attributeName, i,
						Condition.EQ, value));
	...
	int selectFeature() {
		...
		for (int i = 0; i < data.num_attributes - 1; i++) {
			...
			if (temp[i].compareTo("REAL") == 0) {
				...
				entropyArr[i] = (calculateGain() - calculateDiscreteEntropy(i))
						/ splitInfoDiscrete(i);
			}
		}
		int max = 0;
	...
	double calculateDiscreteEntropy(int i) {
		...
		for (int j = 0; j < outputValues.length; j++) {
			...
			double temp = getPartialDataSize(outputValues[j], i)
					/ (float) data.num_instances;
			entropy = entropy + (temp)
					* getEntropy(getPartialData(outputValues[j], i));
		}
		return entropy;
	}
	 ...
	 * 
	 * @param i
	 *            entropy's split info
	 * @return
	 */
	double splitInfo(int i) {
		int total = data.num_instances;
		double entropy = 0;
		entropy = entropy + (-1 * i / (float) total)
				* (Math.log(i / (float) total) / Math.log(2.0));
		entropy = entropy + (-1 * (total - i) / (float) total)
				* (Math.log((total - i) / (float) total) / Math.log(2.0));
		return entropy;
	}
	...
	double calculateContinuousEntropy(int i) {
		...
		for (int j = 0; j < in_attrib.length; j++) {
			for (int k = 0; k < in_attrib.length - 1; k++) {
				if (Double.parseDouble(in_attrib[k]) > Double
						.parseDouble(in_attrib[k + 1])) {
		...
		if (tries > 0) {
			threshcost = (Math.log((double) tries) / (double) size)
					/ Math.log((double) 2);
		} else {
			threshcost = 0;
		}
	 ...
	 * @param out_values
	 *            values of instances for an attribute
	 * @param j
	...
	double calculateThreshold(String[] out_attrib, String[] out_values, int j) {
		...
		double entropy = 0;
		entropy = entropy
				+ ((div1.length / (float) out_attrib.length) * getThreshold(
						div1, out_values));
		entropy = entropy
				+ ((div2.length / (float) out_attrib.length) * getThreshold(
						div2, out_values));
		return entropy;
	}
	...
	double getThreshold(String[] out, String[] in) {
		...
		for (int i = 0; i < in.length; i++) {
			...
			if (tem != 0 && out.length != 0 && tem != 1.0) {
				threshold = threshold + (-tem)
						* (Math.log(tem) / Math.log(2.0));
			} else {
				return 0.0;
			}
		}
	 ...
	 * 
	 * @param v
	 *            vector whose entropy is calculated
	 * @return entropy
	 */
	double getEntropy(Vector v) {
		...
		for (int i = 0; i < outputValues.length; i++) {
			...
			for (int j = 0; j < v.size(); j++) {
				...
				if (temp[data.getClassIndex()].trim().compareTo(
						outputValues[i].trim()) == 0) {
					nk[i]++;
				}
			}
			total += nk[i];
